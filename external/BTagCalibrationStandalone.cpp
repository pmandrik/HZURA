...
throw std::exception();
  ...
  cout << "const std::vector<BTagEntry> &entries => " << entries.size() << endl;

  for (const auto &be : entries) {
    // cout << "add entry " << be.params.jetFlavor << " " << be.formula.c_str();
                                             ...
                                             float discr) const
{
  ...
  if (eta_is_out_of_bounds) {
    // cout << "eta_is_out_of_bounds " << eta << " " << sf_bounds_eta.first << " " << sf_bounds_eta.second << endl;  
    return 1.;
  }
                                               ...
                                               float discr) const
{
  ...
  // cout << "tmpData_.at(jf).size() = " << jf << " " << tmpData_.at(jf).size() << endl;
...
double BTagCalibrationReader::eval_auto_bounds(const std::string & sys,
                                               ...
                                               float discr) const
{
  // std::cout << " BTagCalibrationReader " << pimpl << " " << sys << " " << jf << " " << eta << " " << pt << " " << discr << std::endl;
