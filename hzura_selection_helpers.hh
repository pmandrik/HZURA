...
namespace hzura {
  ...
  void apply_energy_correction( vector<Photon> photons, string type ){
    // https://twiki.cern.ch/twiki/bin/viewauth/CMS/EgammaMiniAODV2#Applying_the_Energy_Scale_and_sm
    for(int i = 0, size = photons.size(); i < size; i++){
      Photon & p = photons[i];
  ...
  void apply_energy_correction( vector<Electron> electrons, string type ){
    // https://twiki.cern.ch/twiki/bin/viewauth/CMS/EgammaMiniAODV2#Applying_the_Energy_Scale_and_sm
    for(int i = 0, size = electrons.size(); i < size; i++){
      Electron & p = electrons[i];
      const int & index = p.index;
